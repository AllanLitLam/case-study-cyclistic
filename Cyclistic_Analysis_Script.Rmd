...
Q1_2019 = readr::read_csv(here::here("Divvy_Trips_2019_Q1.csv"))%>% mutate(year = "2019")
glimpse(Q1_2019) 
Q1_2020 = readr::read_csv(here::here("Divvy_Trips_2020_Q1.csv"))%>% mutate(year = "2020")
glimpse(Q1_2020) 
...
Q1_summary = Q1_agg_trimmed %>% 
  ...
  summarize(group_trips = n(),
            ...
            median_ride_length_sec = median(ride_length_sec, na.rm = TRUE),
...
ggplot(data = Q1_summary, aes(x= usertype, y= mean_ride_length_min, fill = usertype)) + 
  geom_col() + 
  geom_text(aes(label = round(median_ride_length_min,2)), vjust = -0.5) +
  labs(title = "Median Ride Length by User Type", y = "Median Ride Length (minutes)", x = "User Type") +
  ylim(0,85) +
  theme_light()
...
ggplot(data = Q1_summary_by_days, aes(x= usertype, y= median_ride_length_min, fill = usertype)) + 
  geom_col() +
  geom_text(aes(label = round(median_ride_length_min,2)), vjust = -0.2) +
  facet_wrap(~ fct_relevel(day_of_week,
                           "Monday", "Tuesday", "Wednesday", 
                           "Thursday", "Friday", "Saturday", "Sunday"), axes = "all") +
  labs(title = "Median Ride Length by User Type, Separated by Day", y = "Median Ride Length (minutes)", x = "User Type") +
...
ggplot(data = Q1_summary_by_days, aes(x= usertype, y= group_trips, fill = usertype)) + 
  ...
  geom_text(aes(label = scales::comma(group_trips)), vjust = -0.1) +
   facet_wrap(~ fct_relevel(day_of_week,
                           "Monday", "Tuesday", "Wednesday", 
                           "Thursday", "Friday", "Saturday", "Sunday"), axes = "all") +
  labs(title = "Total Trips by User Type, Separated by Day", y = "Median Ride Length (minutes)", x = "User Type") +
...
Q1_agg_trimmed = Q1_agg_trimmed %>% mutate (UserType = 
                                              case_when (usertype == "Customer" ~ "Casual Riders", usertype == "Subscriber" ~ "Annual Members"))
